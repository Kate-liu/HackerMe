...
- DOS（拒绝服务）与 DDOS（分布式拒绝服务） 
  - ![1619078675349](HackerMeWeb.assets/1619078675349.png)
- 危害
...
#### OSI7 层模型 &TCP/IP4 层结构 

![1619078776134](HackerMeWeb.assets/1619078776134.png)
...
![1619079027391](HackerMeWeb.assets/1619079027391.png)
...
- DNS 
  - ![1619079099646](HackerMeWeb.assets/1619079099646.png)
- DNS 放大攻击 
  - 请求包较小，响应包非常大，黑客使用伪造的请求包请求 DNS 响应，IP 改为受害者IP。DNS 服务器 向受害者发送大量的包导致其网络被虚假流量堵塞。 
  - ![1619079149169](HackerMeWeb.assets/1619079149169.png)
...
- Ping 洪水的破坏效果与对目标服务器发出的请求数量成正比。 
  - ![1619079223035](HackerMeWeb.assets/1619079223035.png)
...
- 内存缓存memcached 
  - ![1619079279788](HackerMeWeb.assets/1619079279788.png)
- 内存缓存 DDOS 
  - ![1619079307088](HackerMeWeb.assets/1619079307088.png)



#### 传输层攻击 

- 通常通过 TCP 协议的漏洞，对目标服务器发送大量数据流，使其网络带宽饱和或连接资源耗尽，以至于网络瘫痪，典型的网络层攻击有：
  - SYN 泛洪攻击
  - ACK 泛洪攻击 
- TCP 协议 
  - ![1619079934958](HackerMeWeb.assets/1619079934958.png)
...
- 接收到了客户端的 SYN 包并且发送了 ACK 时的状态变为 SYN_RECV。再进一步接收到客户端的 ACK 就进入 ESTABLISHED 状态，但永远接受不到这个客户端 ACK 确认包。 
  - ![1619080020045](HackerMeWeb.assets/1619080020045.png)
- netstat -an | grep tcp
  - ![1619080055212](HackerMeWeb.assets/1619080055212.png)
...
- 利用合法和非法 ACK 数据包看起来基本相同的特性，进行 DDOS 攻击，不易被外部安全设备拦截的特性，针对于服务器和防火墙的攻击。 
  - ![1619080114551](HackerMeWeb.assets/1619080114551.png)
...
- 黑客发送的正常的（GET POST）请求，针对于服务器需要消耗大量资源的页面，如账号登录，搜索等功能，造成服务器资源耗尽，无法响应正常连接。
  -  ![1619080216854](HackerMeWeb.assets/1619080216854.png)
...
- 去掉一个（\r\n）表示头部未结束，客户端再发送任意头部保持连接，耗尽服务器连接数，造成拒绝服务。 
  - ![1619080356374](HackerMeWeb.assets/1619080356374.png)
  - ![1619080375392](HackerMeWeb.assets/1619080375392.png)
...
- 当多台机器这样连接，耗尽服务器连接资源，造成拒绝服务。 
  - ![1619080426572](HackerMeWeb.assets/1619080426572.png)
...
一个验证凭证的过程，载体如账号密码，cookie（session）、token，数字证书，手机验证码。
...
HTTP 协议是无状态无连接的协议，服务端对于客户端每次发送的请求都认为它是一个新的请求，上一次会话和下一次会话没有联系。
...
![1619081357864](HackerMeWeb.assets/1619081357864.png)
...
- 一家公司受到凭证填充攻击，不一定表示它自身的安全已受损（或密码强度不足），通常关联为另一个网站沦陷。 
  - ![1619081433914](HackerMeWeb.assets/1619081433914.png)
...
- 如：支付转账与订单查询 
  - ![1619081479434](HackerMeWeb.assets/1619081479434.png)
...
- sesionid 过期时间：设置强制过期时间（会话保持攻击） 
- sesionid 位置：
  - URL
  - 隐藏域
  - cookie（相对安全） 

![1619081682088](HackerMeWeb.assets/1619081682088.png)
...
![1619081735083](HackerMeWeb.assets/1619081735083.png)



##### 基于 token 的认证 

![1619081796195](HackerMeWeb.assets/1619081796195.png)



##### JWT(json web token) 

![1619081816021](HackerMeWeb.assets/1619081816021.png)
...
-  ![1619081858150](HackerMeWeb.assets/1619081858150.png)
...
- 管理员
- 普通用户
- ![1619082362667](HackerMeWeb.assets/1619082362667.png)
...
- 水平：用户1 与 用户2
- 垂直：用户1 与 管理员
- ![1619082414400](HackerMeWeb.assets/1619082414400.png)
...
- 用户与角色之间的关系
- ![1619082450696](HackerMeWeb.assets/1619082450696.png)

#### 垂直权限&基于角色的权限控制 

- 最小权限原则 
- ![1619082486078](HackerMeWeb.assets/1619082486078.png)



#### 水平权限&同角色互访问题 

![1619082509596](HackerMeWeb.assets/1619082509596.png)



#### 水平权限&权限控制 

- 根据用户 id 进行不同用户之间的权限控制
- ![1619082570139](HackerMeWeb.assets/1619082570139.png)
...
![1619082696742](HackerMeWeb.assets/1619082696742.png)
...
![1619084738709](HackerMeWeb.assets/1619084738709.png)
...
- 知其所需原则利用需求来给用户授权，仅仅根据 为完成所分配任务 而授权访问 需要操作的数据或资源。
...
基线是一个起点，代表此时处于安全状态，达到通过变更达到下一个安全状态时候，打下下一个基线，保证系统或应用处于安全状态或符合项目流程进度。

基线也用作版本控制，防止由于变更导致的网站瘫痪。 

![1619085169026](HackerMeWeb.assets/1619085169026.png)



#### 变更管理（影响分析&版本控制&配置文档） 

- 变更管理提供一个过程来控制，文档化，跟踪和审计所有系统的变化。
- 主要目标是，变更失败的回退操作，确保变更过程不会导致长时间中断。 

![1619085253267](HackerMeWeb.assets/1619085253267.png)



#### 事件预防与响应 

![1619085276979](HackerMeWeb.assets/1619085276979.png)
...
![1619085312014](HackerMeWeb.assets/1619085312014.png)
...
随着大数据的发展，企业试图谋求更多的个人信息，以提供更好更优质的服务作为理由，但同时，广告根据个人信息精准投放，给企业带来了丰厚的利润，甚至个人信息本身也成了一种商品。 
...
![1619085663681](HackerMeWeb.assets/1619085663681.png)
...
- 网站应该妥善保管收集到的用户数据，不得将数据用于任何指定范围以外的用途。
  - 用户信息仅用于指定用途 
  - **李开复：早期帮助旷世科技拿到蚂蚁金服大量人脸数据做分析** ：https://view.inews.qq.com/wxn2/20200912V0KHSU00
- DNT(Do Not Track) header 
  - 随浏览流量一起发送“不跟踪”请求
  - dnt = 1 
  - ![1619086586702](HackerMeWeb.assets/1619086586702.png)
  - ![1619086604004](HackerMeWeb.assets/1619086604004.png)
...
- http://www.cqn.com.cn/pp/content/2020-05/19/content_8604505.htm
...
匿名模式不是最近才流行的功能，最早提供隐匿模式得是苹果 Safari 浏览器，早在 2005 年，就支持了匿名模式。

Chrome 浏览器在 2008 年就开始支持此模式。



###### DuckDuckGo

2020年5月DuckDuckGo日均搜索次数为6200万。对比看2019年11月底日均搜索次数4900万,2018年10月是2900万。

最近几年的持续活跃度高幅增长，证明了不记录个人隐私的搜素引擎越来越受到用户的青睐。



###### 互联网巨头关注

国内,头条、UC浏览器在搜索框输入状态也提供了「无痕浏览」入口。

不仅是搜索引擎领域，保护用户隐私也成为Facebook最重要的战略方向之一. 

Facebook CEO Mark Zuckerberg在2019年 F8开发者大会上喊出「 THE FUTURE IS PRIVATE J . 2019年3月Mark Zuckerberg发文,主题就是《聚焦于保护隐私的社交网络》。



##### 减少永久性

- 国外社交媒体 Stories （小故事）产品形态的流行
- 人们总是对于所分享的内容永远记录在网上感到担忧。Stories 24小时消失缓解了人们的隐私顾虑,这让用户更安心地自然分享。
- Stories 由Snapchat首创，由Facebook发扬光大。早在2019年4月，Facebook+Messenger Stories, Instagram Stories 日活用户数就突破5亿。
- 2020年2-3月LinkedIn , Twitter也先后宣布将上线类似功能。
...
- 以限定评论互动的公开性为例:
  - 2020年5月Twitter上线了新的评论功能,可以限定谁可以回复帖子的功能,提供了三种选项: 谁都可以评论，只有被关注者可以评论，只有被提及者可以评论三种公开度的限定。
  - Instagram 也在测试「评论限制」新功能，批量屏蔽/限制评论。目前已经上线的一个例子: 用户(评论发布者）如果发布的评论含有攻击性敏感词，发布前伴有提示，提醒评论含有攻击性敏感词是否真的要发布。
- 注重隐私提供 仅好友可见/仅自己可见/仅作者可见/ 等多重维度的隐私设定有助于用户更安心地参与互动。
...
1.匿名模式:
虽然匿名模式由来已久，但仍然是当前的一大基本用户体验设计趋势，尤其是匿名模式的切换便捷性非常重要。

2.减少永久性:
Stories小故事24小时消失缓解了人们的隐私顾虑，这让用户更安心地自然分享,已经成为国外社交媒体平台的必备功能，Facebook, Instagram平台的最主要、最具影响力的功能之一。

3.减少公开性:
用户总是对在社交媒体平台发表评论有所顾忌，限定评论的公开性能够有助于促进用户发帖表出其他用户也可以更安心地参与互动。
...
- 本地搭建钓鱼网站页面
  - ![1619101533946](HackerMeWeb.assets/1619101533946.png)
- 域名伪装
  - ![1619101556207](HackerMeWeb.assets/1619101556207.png)
- 钓鱼网站仿冒对象情况
  - ![1619101590782](HackerMeWeb.assets/1619101590782.png)
...
- 关停站点。
  ...
  - 中国反钓鱼网站联盟（cn 域名） 
    - https://www.cnnic.cn/
    - ![1619101958007](HackerMeWeb.assets/1619101958007.png)
- 自动化识别钓鱼网站。 
  - 钓鱼网站相比真实网站，在技术力上和信息备案有差别，导致了功能缺陷，可用于自动化识别，如：
    - 多次刷新页面。验证码不变。
    - URL 异常
    - 网站的备案信息
    - 支付方式
...
![1619102041277](HackerMeWeb.assets/1619102041277.png)
...
- 参考网址：
  ...
  - https://cloud.tencent.com/developer/article/1459352
- 提供安全培训
- 定义安全需求
  - ![1619144130834](HackerMeWeb.assets/1619144130834.png)
- 定义指标和合规性报告
- 威胁建模
  - ![1619144151059](HackerMeWeb.assets/1619144151059.png)
- 建立设计需求
- 定义和使用的加密标准
  - ![1619144168681](HackerMeWeb.assets/1619144168681.png)
- 管理使用第三方组件的安全风险
- 使用得到授权的工具
  - ![1619144186857](HackerMeWeb.assets/1619144186857.png)
- 静态代码分析
- 动态代码分析
  - ![1619144213496](HackerMeWeb.assets/1619144213496.png)
- 渗透测试
- 建立标准事件响应流程
  - ![1619144224829](HackerMeWeb.assets/1619144224829.png)
...
![1619144607121](HackerMeWeb.assets/1619144607121.png)
...
- 例子
  - 登录界面存在暴力破解：D(3)+ R(3) + E(3) + A(3) + D(3) = 15
  - 密码找回存在逻辑漏洞：D(3)+ R(3) + E(3) + A(3) + D(2) = 14
  - 服务端脚本漏洞（如 SQL 等)： D(3) + R(3)+ E(2) + A(3) + D(1) = 12 
- ![1619144648680](HackerMeWeb.assets/1619144648680.png)



#### WooYun（乌云）

- 官网没了，乌云公开漏洞、知识库搜索网站有，https://wooyun.x10sec.org/
- WooYun 是一个位于厂商和安全研究者之间的安全问题反馈平台，在对安全问题进行反馈处理跟进的同时，为互联网安全研究者提供一个公益、学习、交流和研究的平台。 
- ![1619145325118](HackerMeWeb.assets/1619145325118.png)



#### 应急响应中心 

![1619145349030](HackerMeWeb.assets/1619145349030.png)



#### 流程示意图 

![1619145486265](HackerMeWeb.assets/1619145486265.png)
...
几乎每个重大事件的根源就是对记录和监控不足的利用，攻击者依靠缺乏监控及时响应。暴力破解，扫描器反复扫描。 



#### 安全监控要点 

- 网络设备监控 Network monitoring
  - 安全相关：
    • 带宽使用率
    • 高内存，CPU 使用率
    • TCP 连接数
...
- 应用监控 Application monitoring
  ...
  - 其他信息：
    • 业务处理必要日志记录
    • 日志备份并保留足够时间 
- 服务监控 Services monitoring
  - 服务监控功能是为帮助那些想要在IT 基础设施监控之上，获得更高层面（业务）监控需求的人设计的。
  - 服务可用性。 
...
- 腾讯云短信
  - ![1619146407970](HackerMeWeb.assets/1619146407970.png)
- 邮箱 IP 异常登录通知
  - ![1619146447779](HackerMeWeb.assets/1619146447779.png)
...
- CTF 习题练习
- 靶场实战
  - 谜团靶场平台：https://mituan.zone/#/login
- OWASP 知识扩展
- 社区交流
  - **安定坊CyberSec bilibili**：https://space.bilibili.com/690651381?spm_id_from=333.905.b_7570496e666f.2
